...
//    ((UIWindow *)[[UIApplication sharedApplication].windows lastObject]).hidden = YES;
    
    self.rankingModel = self.rankingArray[self.index];
    ...
    if ([MusicTool sharedMusicTool].currentURL == nil || [MusicTool sharedMusicTool].currentURL != self.rankingModel.RPURL.radio_24_aac) {
        ...
        [MusicTool sharedMusicTool].coverSmallUrl = self.boradModel.playBackgroundPic;
        [MusicTool sharedMusicTool].nowPlayerName = self.rankingModel.rname;
        [[MusicTool sharedMusicTool]play];
        [MusicTool sharedMusicTool].currentURL = self.rankingModel.RPURL.radio_24_aac;
    }
    [MusicTool sharedMusicTool].isLiving = YES;
...
- (void)setProView{
    ...
    [self.broadCastBottomView.backButton addTarget:self action:@selector(backLiving:) forControlEvents:(UIControlEventTouchUpInside)];
    [self.broadCastBottomView.frontButton addTarget:self action:@selector(frontLiving:) forControlEvents:(UIControlEventTouchUpInside)];
    [self.broadCastBottomView.shareIconView.iconButton addTarget:self action:@selector(shareAction:) forControlEvents:(UIControlEventTouchUpInside)];
    
    
    
    AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    appDelegate.eventTypeBlock = ^(UIEvent *event) {
...
- (void)shareAction:(UIButton *)button {
    NSString *shareText = [NSString stringWithFormat:@"%@--%@", self.rankingModel.rname, self.rankingModel.programName];
    [UMSocialSnsService presentSnsIconSheetView:self
                                         appKey:@"563ab7dbe0f55a08a40009aa"
                                      shareText:shareText
                                     shareImage:nil
...
#pragma mark--切换电台
- (void)backLiving:(UIButton *)button{
    self.index -= 1;
    if (self.index < 0) {
        self.index = self.rankingArray.count - 1;
    }
    self.rankingModel = self.rankingArray[self.index];
    self.titleLabel.text = self.rankingModel.rname;
...
- (void)frontLiving:(UIButton *)button{
    self.index += 1;
    if (self.index > self.rankingArray.count - 1) {
        self.index = 0;
    }
    self.rankingModel = self.rankingArray[self.index];
    self.titleLabel.text = self.rankingModel.rname;
...
- (void)backAction:(UIButton *)button {
    ((UIWindow *)[[UIApplication sharedApplication].windows lastObject]).hidden = NO;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"音乐头像" object:self.boradModel.playBackgroundPic];
