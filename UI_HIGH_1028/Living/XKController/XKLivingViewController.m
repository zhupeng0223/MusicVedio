...
- (void)viewDidLoad {
    ...
    self.XKLivingTableView.block1 = ^(XKRankingListModel *model){
        ...
        boardVC.rankingArray = WeakSelf.recommendArray;
        for (int i = 0; i < WeakSelf.recommendArray.count; i++) {
            if (model == WeakSelf.recommendArray[i]) {
                boardVC.index = i;
            }
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 2) {
        ...
        boardVC.rankingArray = self.XKLivingTableView.rankingArray;
        boardVC.index = indexPath.row; 
