...
//    ((UIWindow *)[[UIApplication sharedApplication].windows lastObject]).hidden = YES;
    ...
    [self.voiceButton setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"voice"]]];
    ...
    if (self.tracksArray.count != 0) {
        if ([MusicTool sharedMusicTool].currentURL == nil || [MusicTool sharedMusicTool].currentURL != self.tracksList.playUrl64) {
            [MusicTool sharedMusicTool].nowPlayerName = self.tracksList.title;
            [MusicTool sharedMusicTool].coverSmallUrl = self.tracksList.coverLarge;
            [[MusicTool sharedMusicTool] preparePlayWithMusic:self.tracksList.playUrl64];
            [[MusicTool sharedMusicTool] play];
        }else {
            [[MusicTool sharedMusicTool] play];
        }
        [MusicTool sharedMusicTool].currentURL = self.tracksList.playUrl64;
    }
    ...
    [MusicTool sharedMusicTool].isLiving = NO;
...
- (void)shareAction:(UIButton *)button {
    NSString *shareText = [NSString stringWithFormat:@"%@ \n 歌曲链接%@", ((TracksList *)self.tracksArray[self.currentIndex]).title, ((TracksList *)self.tracksArray[self.currentIndex]).playUrl64];
...
- (void)preAction:(UIButton *)button {
    ...
    [MusicTool sharedMusicTool].nowPlayerName = ((TracksList *)self.tracksArray[self.currentIndex]).title;
    [MusicTool sharedMusicTool].coverSmallUrl = self.tracksList.coverLarge;
...
- (void)backAction:(UIButton *)button {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"音乐头像" object:self.tracksList.coverLarge];
